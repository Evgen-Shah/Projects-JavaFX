	...
	void initialize() {
		...
		reg.setOnAction(event -> {// Назад
			if(!names.getText().matches("[А-Яа-я]*?\\s[А-Яа-я]*?\\s[А-Яа-я]*")) {
				...
				Entry_Reg.phones = phone.getText();
				
				//accs.AddNumberAndPass(phones + ":" + pass.getText());// Добавляем в лист номер телефона и пароль нового пользователя.
				
				try {
					bs.addClients(name, Long.parseLong(phone.getText()), date.getText(), pass.getText());
